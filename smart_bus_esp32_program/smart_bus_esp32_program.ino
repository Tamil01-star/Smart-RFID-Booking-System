...
void loop() {
  ...
  switch (currentState) {
    ...
    case STATE_STANDBY:
      ...
      if (key == 'A') { 
        ticketCount = 1; 
        lastScannedUID = ""; // Manual keypad booking (no pre-scanned card)
    ...
    case STATE_VERIFYING_RFID_API:
      if (stateJustChanged) {
        ...
        if      (code == 200) changeState(STATE_RFID_SCANNED_BOOKED);
          ...
          // FAST BOOKING: No pre-booking today -> directly jump to destination selection!
          currentBoardingIndex = 0; // Current Stop = Salem (Index 0)
          changeState(STATE_WALKIN_PROMPT);
        }
        else {
    ...
    // ----------------------------------------
    // 404: Registered but no ticket today (Unused now, kept for fallback)
    // ----------------------------------------
    ...
    // ----------------------------------------
    // WALK-IN: Select Destination (FAST BOOKING FLOW)
    // ----------------------------------------
    case STATE_WALKIN_PROMPT:
      if (stateJustChanged) {
        currentBoardingIndex = 0; // Salem (Default stop 0)
        currentDestIndex = NUM_STOPS - 1; // Default last stop
        showLCD("Dest (1-9):", String(stopNames[currentDestIndex]) + "  #=OK");
        stateJustChanged = false;
      }
      if (key >= '1' && key <= '9') {
        int idx = key - '1';
        if (idx > currentBoardingIndex && idx < NUM_STOPS) {
          ...
          showLCD("Dest: " + String(stopNames[idx]), "#=Confirm  C=Cxl");
        }
      }
      if (key == '#') {
        // If card was already scanned (lastScannedUID available), PAY IMMEDIATELY!
        if (lastScannedUID != "") {
          ...
          Serial.print("Fast Payment API code: "); Serial.println(code);
          ...
          changeState(STATE_SELECT_CATEGORY);
        }
      }
